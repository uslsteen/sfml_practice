...
Strings* Create_Ptr_Buf(char* buf, size_t buf_size, size_t* str_cnt, char end_value)
{
	assert(buf != NULL);
	assert(buf_size != 0);
	assert(str_cnt != NULL);
	...
	Strings* ptrs_buffer = (Strings*)calloc(num_of_strs, sizeof(ptrs_buffer[0]));
